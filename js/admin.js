...
const search = filmSearch.value
	.trim()
	.toLowerCase();

const filteredFilms = existingFilms.filter(film =>
	String(film.title || "")
		.toLowerCase()
		.includes(search)
);
...
if (!films.length) {
	...
	const search = filmSearch.value.trim();
...
try {
	...
	if (rows.length < 2) {
		...
		alert(
			"Il CSV non contiene abbastanza dati."
		);
	...
	if (!csvFilms.length) {
		...
		alert(
			"Non sono stati trovati film nel CSV."
		);
...
const url =
	document.getElementById("csvImage")
		.value
		.trim();
